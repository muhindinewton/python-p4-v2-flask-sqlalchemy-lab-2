...
class Review(db.Model, SerializerMixin):
    ...
    #Foreign key to store itema id
    item_id = db.Column(db.Integer, db.ForeignKey('items.id'))
    
    #Relationship mapping the review to the customer
    customer = db.relationship('Customer', back_populates = 'reviews')
    
    #Relationship mapping the review to the item
    item = db.relationship('Item', back_populates = 'reviews')

    def __repr__(self):
        return f'<Review {self.id}, {self.comment}, {self.customer.name}, {self.item.name}, {self.item.price}>'